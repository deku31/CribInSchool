...
public class WordGrid : MonoBehaviour
{
    ...
    private Vector2 GetFirsPosition()
    {
        ...
        var midwidthPositon = (((currentGameData.selectBoardData.Columns - 1) * squareSize.x) / 3) * 0.01f;
        var midwidthHegiht = (((currentGameData.selectBoardData.Rows - 1) * squareSize.y) / 3) * 0.01f;
            ...
            foreach (var square in currentGameData.selectBoardData.Board)
            {
                foreach (var squareLetter in square.Row)
                {
                    ...
                    else
                    {
                        ...
                        _squareList[_squareList.Count - 1].GetComponent<Transform>().position = new Vector3(0, 0, -1);
    ...
    {
        float height = cameraPuzzle.orthographicSize * 2;
